...
def main() -> None:
    ...
    parser.add_argument("--out_dir", type=str, default="artifacts_optimized")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of parallel jobs for training components (use 1 on Windows to avoid resource issues)",
    )
    ...
    # On Windows, use threading instead of multiprocessing to avoid MemoryError
    import os
    if os.name == 'nt':
        os.environ["LOKY_MAX_CPU_COUNT"] = "4"  # Limit loky workers
        # Force sklearn/joblib to use threading backend on Windows
        try:
            from sklearn.utils import parallel_backend
            print("⚙️ Configured joblib backend: threading (Windows optimization)")
        except ImportError:
            pass
    ...
    if not args.skip_classifier:
        clf, clf_tuning = train_classifier_optimized(
            parts["train"],
            parts["train"]["has_impact"].astype(int),
            parts["val"],
            parts["val"]["has_impact"].astype(int),
            feature_cols,
            n_iter=args.n_iter,
        ...
        metrics_val = evaluate_classifier(
            clf, parts["val"], parts["val"]["has_impact"].astype(int), feature_cols
        )
        metrics_test = evaluate_classifier(
            clf, parts["test"], parts["test"]["has_impact"].astype(int), feature_cols
        )
    ...
    if not args.skip_regressor:
        ...
        reg, reg_tuning = train_regressor_optimized(
            train_pos,
            train_pos["Affected"].astype(float),
            val_pos,
            val_pos["Affected"].astype(float),
            feature_cols,
            n_iter=args.n_iter,
            cv_folds=args.cv_folds,
            random_state=args.random_state,
        ...
        metrics_val = evaluate_regressor(
            reg, val_pos, val_pos["Affected"].astype(float), feature_cols
        )
        metrics_test = evaluate_regressor(
            reg, test_pos, test_pos["Affected"].astype(float), feature_cols
        )
